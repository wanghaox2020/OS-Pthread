...
int keys[NUM_KEYS];
pthread_rwlock_t spinlock[NUM_BUCKETS]; // declare lock with write premission (spin)
...
void insert(int key, int val)
{
    ...
    pthread_rwlock_wrlock(&spinlock[i]); // aquiring the lock
    e->next = table[i];
    e->key = key;
    e->val = val;
    table[i] = e;
    pthread_rwlock_unlock(&spinlock[i]); // release lock
}
...
int main(int argc, char **argv)
{
    ...
    {
        pthread_rwlock_init(&spinlock[i], NULL);
    }
