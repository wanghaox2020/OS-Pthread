...
int keys[NUM_KEYS];
pthread_mutex_t muxlock[NUM_BUCKETS]; // declare lock (mutex)
...
void insert(int key, int val)
{
    ...
    pthread_mutex_lock(&muxlock[i]); // aquiring the lock
    e->next = table[i];
    e->key = key;
    e->val = val;
    table[i] = e;
    pthread_mutex_unlock(&muxlock[i]); // release lock
}
...
int main(int argc, char **argv)
{
    ...
    {
        pthread_mutex_init(&muxlock[i], NULL);
    }
